...
var AdministrativeApi = require('./provision');
...
var DEFAULT_APIROOT = 'wss://api.organiq.io';
...
function Organiq(options) {
  ...
  var adminApi = new AdministrativeApi(apiRoot, { username: apiKeyId, password: apiKeySecret });
  var container = new DeviceContainer({
    defaultDomain: namespace, aliasLookup: adminApi.getOrCreateDevice});
  ...
  /**
   * Normalize a user-supplied device alias.
   *
   * For convenience, the SDK allows user to supply aliases without being
   * fully qualified.
   *
   * @param {string} alias
   * @return {string} A normalized alias of the form domain:alias
   */
  function normalizeAlias(alias) {
    var parts = alias.toLowerCase().split(':');
    if (parts.length === 1) {
      parts[1] = parts[0];
      parts[0] = namespace;
    }
  ...
  this.registerDevice = function (alias, impl, schema) {
    ...
    var d$ = adminApi.getOrCreateDeviceByAlias(alias);
