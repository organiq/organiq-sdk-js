...
function AdministrativeApi(apiRoot, options) {
  ...
  this.getOrCreateDeviceByAlias = function (alias) {
    return _getDeviceId(alias).then(null, function (err) {
      void(err);
      var productName = 'AutoProvisioned-' + alias;
      return _getOrCreateProduct(productName).then(function(productId) {
        return _createDevice(alias, productId).then(function () {
  ...
  this.getOrCreateProductByName = function (name) {
    return _getProductByAlias(name).then(null, function (err) {
      void(err);
      var productName = 'AutoProvisioned-' + alias;
      return _getOrCreateProduct(productName).then(function(productId) {
        return _createDevice(alias, productId).then(function () {
